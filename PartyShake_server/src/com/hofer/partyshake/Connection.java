...
public class Connection {
	...
	public Element receive_msg_package() {
		try {
			...
			InputStream XML_package_stream = new ByteArrayInputStream(
					XML_package.getBytes("UTF-8"));
	...
	public void get_login_info(Element root, String client_name) {
		Element temp = (Element) root.getElementsByTagName("client_name").item(
				0);
	...
	public void get_search_party_info(Element root, Position p) {
		...
		p.longitude = Double.parseDouble(p_root
				.getElementsByTagName("latitude").item(0).getFirstChild()
				.getNodeValue());
		p.latitude = Double.parseDouble(p_root
				.getElementsByTagName("longitude").item(0).getFirstChild()
				.getNodeValue());
		p.position_discript = p_root.getElementsByTagName("position_discript")
				.item(0).getFirstChild().getNodeValue();
	}

	public void get_party_setup_info(Element root, Server_party Sp) {
		Element temp = (Element) root.getElementsByTagName("party_Name")
				.item(0);
		Sp.party_name = temp.getFirstChild().getNodeValue();

		temp = (Element) root.getElementsByTagName("party_date").item(0);
		Sp.party_date = new Date(Long.parseLong(temp.getFirstChild()
				.getNodeValue()));
		...
		NodeList e_list = root.getElementsByTagName("party_position");
		Element p_root = (Element) e_list.item(0);
		Sp.party_position.longitude = Double.parseDouble(p_root
				.getElementsByTagName("latitude").item(0).getFirstChild()
				.getNodeValue());
		Sp.party_position.latitude = Double.parseDouble(p_root
				.getElementsByTagName("longitude").item(0).getFirstChild()
				.getNodeValue());
		Sp.party_position.position_discript = p_root
				.getElementsByTagName("position_discript").item(0)
	...
	public void get_join_party_info(Element root, int party_id,
			String client_name) {
		party_id = Integer.parseInt(root.getAttribute("party_id"));
	...
	public boolean reply_party_setup(String client_name, Server_party sp) {
		try {
			...
			String Date = "<party_date>" + sp.party_date.getTime()
					+ "</party_date>";
			String drt = "<party_duration>" + sp.duration + "</party_duration>";
			String p_pos = "<party_position>" + "<longitude>"
					+ sp.party_position.longitude + "</longitude>"
					+ "<latitude>" + sp.party_position.latitude + "</latitude>"
					+ "<position_discript>"
					+ sp.party_position.position_discript
					+ "</position_discript>" + "</party_position>";
			buffer = "<msg>" + buffer + name + p_id + Date + drt + p_pos
					+ "</msg>";
			XML_package += buffer;
			dos.writeUTF(XML_package);
			dos.flush();
	...
	public boolean reply_search_party(Server_party Sp) {
		try {
			...
			String p_pos = "<party_position>" + "<longitude>"
					...
					+ "</position_discript>" + "</party_position>";
			String p_id = "<party_id>" + Sp.party_id + "</party_id>";
			buffer = "<msg>" + buffer + name + Date + drt + p_pos + p_id
					+ "</msg>";
			XML_package += buffer;
			dos.writeUTF(XML_package);
			dos.flush();
